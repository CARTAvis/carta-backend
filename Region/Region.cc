...
Region::Region(const RegionInfo& info, casacore::CoordinateSystem* csys)
    : _coord_sys(nullptr), _valid(false), _region_changed(false), _ref_region_set(false), _z_profile_count(0) {
    _valid = UpdateRegion(info, csys);
}
...
bool Region::UpdateRegion(const RegionInfo& info, casacore::CoordinateSystem* csys) {
    ...
    if (valid) {
        ...
        if (_region_changed) {
            ...
            if (_applied_regions.count(info.reference_file_id)) {
                _applied_regions.at(info.reference_file_id).reset();
            }
        }

        // set new region info and coord sys
        _region_info = info;
        delete _coord_sys;
        _coord_sys = csys;
...
casacore::TableRecord Region::GetImageRegionRecord(
    ...
    if (file_id == _region_info.reference_file_id) {
        ...
        bool is_rotbox((_region_info.type == CARTA::RegionType::RECTANGLE) && (_region_info.rotation != 0.0));
...
casacore::TableRecord Region::GetPolygonRecord(const casacore::CoordinateSystem& output_csys) {
    ...
    try {
        ...
        if (_coord_sys->hasDirectionCoordinate() && output_csys.hasDirectionCoordinate()) {
            ...
            for (size_t i = 0; i < _wcs_control_points.size(); i += 2) {
                // For each control point, convert reference world coord to output coord sys
                casacore::MDirection world_direction(_wcs_control_points[i], _wcs_control_points[i + 1], reference_dir_type);
                if (reference_dir_type != output_dir_type) {
                    world_direction = casacore::MDirection::Convert(world_direction, output_dir_type)();
                }
