//
// From the original file: "casacore/lattices/LatticeMath/LatticeApply.tcc"
//
#ifndef CARTA_BACKEND_ANALYSIS_MYLATTICEAPPLY_TCC_
#define CARTA_BACKEND_ANALYSIS_MYLATTICEAPPLY_TCC_
...
void MyLatticeApply<T, U>::lineApply(MaskedLattice<U>& latticeOut, const MaskedLattice<T>& latticeIn, const LatticeRegion& region,
    LineCollapser<T, U>& collapser, uInt collapseAxis, LatticeProgress* tellProgress) {
    lineApply(latticeOut, SubLattice<T>(latticeIn, region), collapser, collapseAxis, tellProgress);
}

template <class T, class U>
void MyLatticeApply<T, U>::lineMultiApply(PtrBlock<MaskedLattice<U>*>& latticeOut, const MaskedLattice<T>& latticeIn,
    const LatticeRegion& region, LineCollapser<T, U>& collapser, uInt collapseAxis, LatticeProgress* tellProgress) {
    lineMultiApply(latticeOut, SubLattice<T>(latticeIn, region), collapser, collapseAxis, tellProgress);
}

template <class T, class U>
void MyLatticeApply<T, U>::tiledApply(MaskedLattice<U>& latticeOut, const MaskedLattice<T>& latticeIn, const LatticeRegion& region,
    TiledCollapser<T, U>& collapser, const IPosition& collapseAxes, Int newOutAxis, LatticeProgress* tellProgress) {
    tiledApply(latticeOut, SubLattice<T>(latticeIn, region), collapser, collapseAxes, newOutAxis, tellProgress);
}

template <class T, class U>
void MyLatticeApply<T, U>::lineApply(MaskedLattice<U>& latticeOut, const MaskedLattice<T>& latticeIn, LineCollapser<T, U>& collapser,
    ...
    IPosition ioMap = prepare(latticeIn.shape(), latticeOut.shape(), IPosition(1, collapseAxis), -1);
...
template <class T, class U>
IPosition MyLatticeApply<T, U>::_chunkShape(uInt axis, const MaskedLattice<T>& latticeIn) {
...
template <class T, class U>
void MyLatticeApply<T, U>::tiledApply(MaskedLattice<U>& latticeOut, const MaskedLattice<T>& latticeIn, TiledCollapser<T, U>& collapser,
    const IPosition& collapseAxes, Int newOutAxis, LatticeProgress* tellProgress) {
    ...
    while (!inIter.atEnd()) {
        ...
        // Ditto for the mask
        const Array<T>& cursor = iterCursor.contiguousStorage() ? iterCursor : iterCursor.copy();
        ThrowIf(!cursor.contiguousStorage(), "cursor array is not contiguous");
        ...
        if (useMask) {
            ...
            if (!mask.contiguousStorage()) {
                mask = mask.copy();
                ThrowIf(!mask.contiguousStorage(), "mask array is not contiguous");
            }
        }
        for (j = 0; j < outDim; ++j) {
            if (ioMap(j) >= 0) {
        ...
        for (;;) {
            for (;;) {
                if (useMask) {
                    collapser.process(index1, index3, &(cursor(curPos)), &(mask(curPos)), dataIncr, maskIncr, nval, latPos, chunkShape);
                } else {
                    collapser.process(index1, index3, &(cursor(curPos)), 0, dataIncr, maskIncr, nval, latPos, chunkShape);
                }
...
template <class T, class U>
IPosition MyLatticeApply<T, U>::prepare(
    const IPosition& inShape, const IPosition& outShape, const IPosition& collapseAxes, Int newOutAxis) {
    uInt i;
    ...
    for (i = collDim; i < inDim; ++i) {
        ...
        if (inShape(axis) != outShape(k)) {
            throw(
                AipsError("LatticeApply::prepare - "
...
#endif // CARTA_BACKEND_ANALYSIS_MYLATTICEAPPLY_TCC_