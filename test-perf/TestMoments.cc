...
int main(int argc, char* argv[]) {
    ...
    // Set region moment request
    auto spectral_range = Message::IntBounds(0, frame->Depth() - 1);
    auto pixel_range = Message::FloatBounds(0, 1);
    auto moment_request =
        Message::MomentsRequest(file_id, region_id, CARTA::MomentAxis::SPECTRAL, CARTA::MomentMask::None, spectral_range, pixel_range);

    // Set region moment response
    auto progress_callback = [&](float progress) {};
    CARTA::MomentResponse moment_response;
    std::vector<GeneratedImage> collapse_results;

    // Do moments calculation
    carta::Timer t;
    EXPECT_TRUE(
        region_handler->CalculateMoments(file_id, region_id, frame, progress_callback, moment_request, moment_response, collapse_results));
    spdlog::info("Elapsed time to calculate region moments: {} (ms)", t.Elapsed().ms());

    EXPECT_EQ(collapse_results.size(), moment_request.moments().size() - 1);
