...
Frame::Frame(uint32_t session_id, carta::FileLoader* loader, const std::string& hdu, bool verbose, int default_channel)
    ...
    if (!_loader) {
        ...
        if (_verbose) {
            CartaLog(session_id, _open_image_error);
        }
        _valid = false;
        return;
    }
...
bool Frame::SetImageCache() {
    ...
    try {
        ...
        CartaLog(_session_id, "Could not allocate memory for image data.");
        return false;
    }
    ...
    if (!_loader->GetSlice(tmp, section)) {
        CartaLog(_session_id, "Loading image cache failed.");
        return false;
        ...
        if (region->IsValid()) {
            ...
            if (region->GetRegion(image_region, stokes, channel_range)) {
                try {
                    ...
                    CartaLog(_session_id, "Region creation for {} failed: {}", region->Name(), err.getMesg());
                }
            }
        }
    }
