...
TEST_F(PvGeneratorTest, FitsPvImageHorizontalCut) {
    ...
    // Check data
    casacore::Array<float> pv_data, image_data;
    pv_image.image->get(pv_data);
    EXPECT_EQ(pv_data.shape().size(), 2);
    EXPECT_EQ(pv_data.shape()(0), 9);
    EXPECT_EQ(pv_data.shape()(1), frame->Depth());

    auto image_slicer = frame->GetImageSlicer(AxisRange(1, 9), AxisRange(5), AxisRange(ALL_Z), CURRENT_STOKES);
    image_data.resize(image_slicer.length());
    loader->GetSlice(image_data, image_slicer);
    image_data.removeDegenerate();

    EXPECT_EQ(pv_data.size(), image_data.size());
    EXPECT_EQ(pv_data.shape(), image_data.shape());
    EXPECT_THAT(pv_data.tovector(), Pointwise(FloatNear(1e-5), image_data.tovector()));
}
