...
fs::path XmlTables() {
    return TestRoot() / "data" / "tables" / "xml";
}
...
TEST_F(VoTableTest, ParseArrayFile) {
    Table table(XmlTables() / "array_types.xml");
    EXPECT_TRUE(table.IsValid());
    EXPECT_EQ(table.NumRows(), 3);
}
