...
TEST_F(Hdf5ImageTest, BasicLoadingTest) {
    auto path_string = (TestRoot() / "carta-backend-test-data" / "images" / "hdf5" / "10_10_row_column.hdf5");
...
TEST_F(Hdf5ImageTest, CorrectShapeDegenerate3dImages) {
    auto path_string = (TestRoot() / "carta-backend-test-data" / "images" / "hdf5" / "10_10_10_1_row_column.hdf5");
    std::shared_ptr<carta::FileLoader> loader(carta::FileLoader::GetLoader(path_string));
    std::unique_ptr<Frame> frame(new Frame(0, loader, "0"));
    EXPECT_TRUE(frame->IsValid());
    ...
    path_string = (TestRoot() / "carta-backend-test-data" / "images" / "hdf5" / "10_10_1_10_row_column.hdf5");
...
TEST_F(Hdf5ImageTest, CorrectShape4dImages) {
    auto path_string = (TestRoot() / "carta-backend-test-data" / "images" / "hdf5" / "10_10_5_2_row_column.hdf5");
    std::shared_ptr<carta::FileLoader> loader(carta::FileLoader::GetLoader(path_string));
    std::unique_ptr<Frame> frame(new Frame(0, loader, "0"));
    EXPECT_TRUE(frame->IsValid());
    ...
    // CASA-generated images often have spectral and Stokes axes swapped
    path_string = (TestRoot() / "carta-backend-test-data" / "images" / "hdf5" / "10_10_2_5_row_column.hdf5");
    loader.reset(carta::FileLoader::GetLoader(path_string));
    frame.reset(new Frame(0, loader, "0"));
    EXPECT_TRUE(frame->IsValid());
