...
bool FileLoader::FindShape(IPos& shape, const CARTA::FileInfoExtended* info, int& spectral_axis, int& stokes_axis, std::string& message) {
    // Find image shape, spectral axis, and stokes axis from image, coordinate system, and extended file info
    // set defaults
    spectral_axis = -1;
    stokes_axis = -1;
