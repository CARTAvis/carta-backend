    ...
    if (is_lel_expr) {
        ...
        if (info_loaded) {
            ...
            for (int i = 0; i < file_info_extended.computed_entries_size(); ++i) {
                auto entry = file_info_extended.computed_entries(i);
                if (entry.name() == "Data type") {
                    if ((entry.value() == "Complex") || (entry.value() == "DComplex")) {
                    ...
                    break; // not complex data type, continue loading image
                }
            }
