...
Histogram::Histogram(int num_bins, float min_value, float max_value, const std::vector<float>& data)
    : _num_bins(num_bins), _bin_width((max_value - min_value) / num_bins), _min_val(min_value), _max_val(max_value), _hist(num_bins, 0), _data(data) {}

Histogram::Histogram(Histogram& h, tbb::split)
    : _num_bins(h._num_bins), _bin_width(h._bin_width), _min_val(h._min_val), _max_val(h._max_val), _hist(h._hist.size(), 0), _data(h._data) {}
