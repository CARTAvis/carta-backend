...
>>>>>>> origin/dev
    ...
    bool RegionHandler::HasSpatialRequirements(int region_id, int file_id, const std::string& coordinate, int width) {
        ...
        if (!has_req) {
            spdlog::debug(
                "Spatial requirements for region {} file {} coordinate {} width {} removed", region_id, file_id, coordinate, width);
        }
    ...
    bool RegionHandler::SetSpectralRequirements(int region_id, int file_id, std::shared_ptr<Frame> frame,
        ...
        if (_spectral_req.count(config_id) && !_spectral_req[config_id].configs.empty()) {
            ...
            current_configs.insert(
                current_configs.begin(), _spectral_req[config_id].configs.begin(), _spectral_req[config_id].configs.end());
            ulock.unlock();
    ...
    bool RegionHandler::CalculatePvPreviewImage(int file_id, int region_id, int line_width, AxisRange& spectral_range, bool reverse,
        ...
        if (_pv_preview_cuts.find(preview_id) != _pv_preview_cuts.end() &&
            _pv_preview_cuts.at(preview_id)->HasSameParameters(cut_parameters)) {
        ...
        return CalculatePvPreviewImage(
            frame_id, preview_id, quick_update, preview_cut, preview_cube, progress_callback, pv_response, pv_image);
    }
    ...
    bool RegionHandler::CalculatePvImage(int file_id, int region_id, int width, AxisRange& spectral_range, bool reverse, bool keep,
        ...
        if (GetLineProfiles(file_id, region_id, width, spectral_range, per_z, stokes_index, "", progress_callback, pv_data,
                offset_increment, cancelled, message, reverse)) {
    ...
    bool RegionHandler::GetRegionSpectralData(int region_id, int file_id, const AxisRange& z_range, std::string& coordinate,
        int stokes_index, std::vector<CARTA::StatsType>& required_stats, bool report_error,
            ...
            if (!mask.shape().empty()) {
                ...
                while (progress < 1.0) {
                    ...
                    auto get_profiles_data = [&](ProfilesMap& tmp_results, std::string tmp_coordinate) {
                        int tmp_stokes;
                        return (_frames.at(file_id)->GetStokesTypeIndex(tmp_coordinate, tmp_stokes) &&
                                _frames.at(file_id)->GetLoaderSpectralData(
                                    region_id, z_range, tmp_stokes, mask, xy_origin, tmp_results, progress));
                    };
        ...
        for (const auto& [stats_region_id, region_statistics] : _region_statistics) {
            ...
            for (int stats_file_id : config_file_ids) {
                ...
                for (auto& stats_config : stats_configs) {
                    ...
                    int stokes(0);
                    if (!frame->GetStokesTypeIndex(stats_config.coordinate(), stokes)) {
    ...
    bool RegionHandler::FillLineSpatialProfileData(
        int file_id, int region_id, std::function<void(CARTA::SpatialProfileData profile_data)> cb) {
