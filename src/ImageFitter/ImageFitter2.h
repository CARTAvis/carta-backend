...
template <class T>
class ImageFitter2 : public casa::ImageTask<T> {
    // <summary>
...
public:
    ...
    ImageFitter2(const SPCIIT image, const casacore::String& region, const casacore::Record* const& regionRec,
...
private:
    ...
    void _fitLoop(casacore::Bool& anyConverged, casa::ComponentList& convolvedList, casa::ComponentList& deconvolvedList,
        SPIIT templateImage, SPIIT residualImage, SPIIT modelImage, casacore::String& resultsString);
    ...
    void _fitsky(casacore::Fit2D& fitter, casacore::Array<T>& pixels, casacore::Array<casacore::Bool>& pixelMask, casacore::Bool& converged,
        casacore::Double& zeroLevelOffsetSolution, casacore::Double& zeroLevelOffsetError,
        std::pair<casacore::Int, casacore::Int>& pixelOffsets, const casacore::Vector<casacore::String>& models, casacore::Bool fitIt,
        casacore::Bool deconvolveIt, casacore::Double zeroLevelEstimate);

    casacore::Vector<casacore::Double> _singleParameterEstimate(casacore::Fit2D& fitter, casacore::Fit2D::Types model,
        const casacore::MaskedArray<T>& pixels, T minVal, T maxVal, const casacore::IPosition& minPos,
        const casacore::IPosition& maxPos) const;

    casa::ComponentType::Shape _convertModelType(casacore::Fit2D::Types typeIn) const;

    void _fitskyExtractBeam(casacore::Vector<casacore::Double>& parameters, const casacore::ImageInfo& imageInfo,
        const casacore::Bool xIsLong, const casacore::CoordinateSystem& cSys) const;

    void _encodeSkyComponentError(casa::SkyComponent& sky, casacore::Double facToJy, const casacore::CoordinateSystem& csys,
        const casacore::Vector<casacore::Double>& parameters, const casacore::Vector<casacore::Double>& errors,
        casacore::Stokes::StokesTypes stokes, casacore::Bool xIsLong) const;

    void _doConverged(casa::ComponentList& convolvedList, casa::ComponentList& deconvolvedList, casacore::Double& zeroLevelOffsetEstimate,
        std::pair<casacore::Int, casacore::Int>& pixelOffsets, SPIIT& residualImage, SPIIT& modelImage,
        std::shared_ptr<casacore::TempImage<T>>& tImage, std::shared_ptr<casacore::ArrayLattice<casacore::Bool>>& initMask,
        casacore::Double zeroLevelOffsetSolution, casacore::Double zeroLevelOffsetError, casacore::Bool hasSpectralAxis,
        casacore::Int spectralAxisNumber, casacore::Bool outputImages, const casacore::IPosition& planeShape,
        const casacore::Array<T>& pixels, const casacore::Array<casacore::Bool>& pixelMask, const casacore::Fit2D& fitter);
    ...
    casacore::Double _correlatedOverallSNR(
        casacore::uInt comp, casacore::Double a, casacore::Double b, casacore::Double signalToNoise) const;

    casacore::GaussianBeam _getCurrentBeam() const;

    void _createOutputRecord(const casa::ComponentList& convolved, const casa::ComponentList& decon);

    void _setSum(const casa::SkyComponent& comp, const casacore::SubImage<T>& im, casacore::uInt compNum);
