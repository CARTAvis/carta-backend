...
template <class T> const String ImageFitter2<T>::_class = "ImageFitter";

template <class T> ImageFitter2<T>::ImageFitter2(
...
template <class T> std::pair<casa::ComponentList, casa::ComponentList> ImageFitter2<T>::fit() {
...
template <class T> void ImageFitter2<T>::_createOutputRecord(
...
template <class T> void ImageFitter2<T>::_fitLoop(
...
template <class T> void ImageFitter2<T>::_doConverged(
...
template <class T> void ImageFitter2<T>::setZeroLevelEstimate(
...
template <class T> void ImageFitter2<T>::unsetZeroLevelEstimate() {
    _doZeroLevel = false;
    _zeroLevelOffsetEstimate = 0;
    _zeroLevelIsFixed = false;
}

template <class T> void ImageFitter2<T>::getZeroLevelSolution(
...
template <class T> void ImageFitter2<T>::setRMS(const Quantity& rms) {
...
template <class T> void ImageFitter2<T>::setNoiseFWHM(Double d) {
...
template <class T> Quantity ImageFitter2<T>::_pixelWidth() {
...
template <class T> void ImageFitter2<T>::clearNoiseFWHM() {
    _noiseFWHM.reset();
...
template <class T> void ImageFitter2<T>::setNoiseFWHM(const Quantity& q) {
    ThrowIf(
        ! q.isConform("rad"),
...
template <class T> Double ImageFitter2<T>::_correlatedOverallSNR(
...
template <class T> GaussianBeam ImageFitter2<T>::_getCurrentBeam() const {
    return this->_getImage()->imageInfo().restoringBeam(
        _chanPixNumber, _stokesPixNumber
    );
}

template <class T> void ImageFitter2<T>::_calculateErrors() {
...
template <class T> void ImageFitter2<T>::_setIncludeExclude(Fit2D& fitter) const {
...
template <class T> Bool ImageFitter2<T>::converged(uInt plane) const {
    ThrowIf(! _fitDone, "fit has not yet been performed");
    return _fitConverged[plane];
}

template <class T> Vector<Bool> ImageFitter2<T>::converged() const {
    return _fitConverged;
}

template <class T> void ImageFitter2<T>::_getStandardDeviations(Double& inputStdDev, Double& residStdDev) const {
    inputStdDev = _getStatistic("sigma", _curChan - _chanVec[0], inputStats);
    residStdDev = _getStatistic("sigma", 0, _residStats);
}

template <class T> void ImageFitter2<T>::_getRMSs(Double& inputRMS, Double& residRMS) const {
    inputRMS = _getStatistic("rms", _curChan - _chanVec[0], inputStats);
    residRMS = _getStatistic("rms", 0, _residStats);
}

template <class T> Double ImageFitter2<T>::_getStatistic(const String& type, const uInt index, const Record& stats) const {
    Vector<Double> statVec;
    stats.get(stats.fieldNumber(type), statVec);
    return statVec[index];
}

template <class T> vector<casa::OutputDestinationChecker::OutputStruct> ImageFitter2<T>::_getOutputStruct() {
...
template <class T> vector<Coordinate::Type> ImageFitter2<T>::_getNecessaryCoordinates() const {
    vector<Coordinate::Type> coordType(1);
    coordType[0] = Coordinate::DIRECTION;
    return coordType;
}

template <class T> casa::CasacRegionManager::StokesControl ImageFitter2<T>::_getStokesControl() const {
    return casa::CasacRegionManager::USE_FIRST_STOKES;
}

template <class T> void ImageFitter2<T>::_finishConstruction(const String& estimatesFilename) {
    *this->_getLog() << LogOrigin(_class, __func__);
    ...
    if(estimatesFilename.empty()) {
        ...
        *this->_getLog() << LogIO::NORMAL << "File " << estimatesFilename
...
template <class T> String ImageFitter2<T>::_resultsToString(uInt nPixels) {
...
template <class T> String ImageFitter2<T>::_statisticsToString() const {
    ostringstream stats;
...
template <class T> Double ImageFitter2<T>::_getRMS() const {
    if (_rms > 0) {
        return _rms;
    }
    else {
...
template <class T> void ImageFitter2<T>::_setDeconvolvedSizes() {
    if (! this->_getImage()->imageInfo().hasBeam()) {
        return;
    }
...
template <class T> String ImageFitter2<T>::_sizeToString(const uInt compNumber) const {
    ostringstream size;
...
template <class T> String ImageFitter2<T>::_spectrumToString(uInt compNumber) const {
...
template <class T> SPIIT ImageFitter2<T>::_createImageTemplate() const {
    auto x = casa::SubImageFactory<T>::createImage(
...
template <class T> void ImageFitter2<T>::_fitsky(
    ...
    *this->_getLog() << origin;
    ...
    if (! fitIt) {
        ...
        Double facToJy;
        result(0) = casa::SkyComponentFactory::encodeSkyComponent(
        ...
        if (nEstimates == 0 && modelType == Fit2D::GAUSSIAN) {
            ...
            *this->_getLog() << origin;
            ...
            if (modelType == Fit2D::GAUSSIAN) {
                parameters = casa::SkyComponentFactory::decodeSkyComponent(
            ...
            if (
                ...
                && estType == casa::ComponentType::POINT
            ) {
    ...
    *this->_getLog() << LogOrigin(_class, __func__);
    ...
    for (uInt i = 0; i < nModels; ++i) {
        if (fitter.type(i) == Fit2D::LEVEL) {
            ...
            casa::ComponentType::Shape modelType = _convertModelType(
...
template <class T> void ImageFitter2<T>::_setBeam(GaussianBeam& beam, uInt ngauss) {
    if (beam.isNull()) {
        return;
    }
...
template <class T> void ImageFitter2<T>::_setSum(
    const casa::SkyComponent& comp, const SubImage<T>& im, uInt compNum
) {
    ...
    auto tmp = casa::SubImageFactory<T>::createImage(
...
template <class T> Vector<Double> ImageFitter2<T>::_singleParameterEstimate(
...
template <class T> casa::ComponentType::Shape ImageFitter2<T>::_convertModelType(Fit2D::Types typeIn) const {
...
template <class T> void ImageFitter2<T>::_fitskyExtractBeam(
