...
template <typename T>
template <typename S>
const casacore::IPosition Hdf5Loader<T>::GetStatsDataShapeTyped(FileInfo::Data ds) {
...
casacore::ArrayBase* Hdf5Loader<T>::GetStatsDataTyped(FileInfo::Data ds) {
...
template <typename T>
[[maybe_unused]] Hdf5Loader<T>::Hdf5Loader(const std::string& filename) : FileLoader<T>(filename), _hdu("0") {}

template <typename T>
void Hdf5Loader<T>::OpenFile(const std::string& hdu) {
    // Explicitly handle empty HDU as the default 0
    std::string selected_hdu = hdu.empty() ? "0" : hdu;

    // Open hdf5 image with specified hdu
    if (!this->_image || (selected_hdu != _hdu)) {
        auto hdf5_image = new CartaHdf5Image(this->_filename, DataSetToString(FileInfo::Data::Image), selected_hdu);
        this->_image.reset(hdf5_image);
        if (!this->_image) {
        ...
        if (HasData(FileInfo::Data::SWIZZLED)) {
            _swizzled_image = std::unique_ptr<casacore::HDF5Lattice<float>>(
                new casacore::HDF5Lattice<float>(casacore::CountedPtr<casacore::HDF5File>(new casacore::HDF5File(this->_filename)),
...
template <typename T>
bool Hdf5Loader<T>::HasData(std::string ds_name) const {
    if (!this->_image) {
        return false;
    }
    CartaHdf5Image* hdf5_image = dynamic_cast<CartaHdf5Image*>(this->_image.get());
...
template <typename T>
bool Hdf5Loader<T>::HasData(FileInfo::Data ds) const {
    switch (ds) {
        case FileInfo::Data::Image:
            return true;
        case FileInfo::Data::XY:
            return this->_num_dims >= 2;
        case FileInfo::Data::XYZ:
            return this->_num_dims >= 3;
        case FileInfo::Data::XYZW:
            return this->_num_dims >= 4;
        case FileInfo::Data::MASK:
            return this->_has_pixel_mask;
        default:
...
template <typename T>
casacore::Lattice<float>* Hdf5Loader<T>::LoadSwizzledData() {
    // swizzled data returns a Lattice
    return _swizzled_image.get();
}

template <typename T>
casacore::Lattice<float>* Hdf5Loader<T>::LoadMipMapData(int mip) {
    // mipmap data returns a Lattice
    return _mipmaps[mip].get();
}

template <typename T>
std::string Hdf5Loader<T>::DataSetToString(FileInfo::Data ds) const {
    ...
    switch (ds) {
        case FileInfo::Data::XY:
            return this->_num_dims == 2 ? DataSetToString(FileInfo::Data::Image) : "";
        case FileInfo::Data::XYZ:
            return this->_num_dims == 3 ? DataSetToString(FileInfo::Data::Image) : "";
        case FileInfo::Data::XYZW:
            return this->_num_dims == 4 ? DataSetToString(FileInfo::Data::Image) : "";
        case FileInfo::Data::SWIZZLED:
            switch (this->_num_dims) {
                case 2:
...
template <typename T>
bool Hdf5Loader<T>::HasMip(int mip) const {
...
template <typename T>
const casacore::IPosition Hdf5Loader<T>::GetStatsDataShape(FileInfo::Data ds) {
    auto image = this->GetImage();
    if (!image) {
        return casacore::IPosition();
    }
...
// complicated templating, at least for now.
template <typename T>
casacore::ArrayBase* Hdf5Loader<T>::GetStatsData(FileInfo::Data ds) {
    auto image = this->GetImage();
    if (!image) {
...
template <typename T>
bool Hdf5Loader<T>::GetCursorSpectralData(
    ...
    if (has_swizzled) {
        ...
        if (this->_num_dims == 4) {
            slicer = casacore::Slicer(
                casacore::IPosition(4, 0, cursor_y, cursor_x, stokes), casacore::IPosition(4, this->_depth, count_y, count_x, 1));
        } else if (this->_num_dims == 3) {
            slicer =
                casacore::Slicer(casacore::IPosition(3, 0, cursor_y, cursor_x), casacore::IPosition(3, this->_depth, count_y, count_x));
        }

        data.resize(this->_depth * count_y * count_x);
...
template <typename T>
bool Hdf5Loader<T>::UseRegionSpectralData(const casacore::IPosition& region_shape, std::mutex& image_mutex) {
    std::unique_lock<std::mutex> ulock(image_mutex);
    bool has_swizzled = HasData(FileInfo::Data::SWIZZLED);
    ulock.unlock();
    if (!has_swizzled) {
...
template <typename T>
bool Hdf5Loader<T>::GetRegionSpectralData(int region_id, int stokes, const casacore::ArrayLattice<casacore::Bool>& mask,
    ...
    int depth = this->_depth;
    double beam_area = this->CalculateBeamArea();
...
template <typename T>
bool Hdf5Loader<T>::GetDownsampledRasterData(
...
template <typename T>
bool Hdf5Loader<T>::GetChunk(
    std::vector<float>& data, int& data_width, int& data_height, int min_x, int min_y, int z, int stokes, std::mutex& image_mutex) {
    bool data_ok(false);

    data_width = std::min(CHUNK_SIZE, (int)this->_width - min_x);
    data_height = std::min(CHUNK_SIZE, (int)this->_height - min_y);
...
template <typename T>
bool Hdf5Loader<T>::UseTileCache() const {
