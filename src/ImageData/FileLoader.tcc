...
template <typename T>
FileLoader<T>* FileLoader<T>::GetLoader(const std::string& filename, const std::string& directory) {
    if (!directory.empty()) {
        // filename is LEL expression for image(s) in directory
        return new ExprLoader<T>(filename, directory);
    } else if (IsCompressedFits(filename)) {
        return new FitsLoader<T>(filename, true);
    }

    switch (CasacoreImageType(filename)) {
        case casacore::ImageOpener::AIPSPP:
            return new CasaLoader<T>(filename);
        case casacore::ImageOpener::FITS:
            return new FitsLoader<T>(filename);
        case casacore::ImageOpener::MIRIAD:
            return new MiriadLoader<T>(filename);
        ...
        case casacore::ImageOpener::IMAGEEXPR:
            return new ExprLoader<T>(filename);
        case casacore::ImageOpener::COMPLISTIMAGE:
            return new CompListLoader<T>(filename);
        default:
            break;
    }
    return nullptr;
...
template <typename T>
FileLoader<T>* FileLoader<T>::GetLoader(ImageRef image) {
    if (image) {
        return new ImagePtrLoader<T>(image);
    } else {
...
template <typename T>
FileLoader<T>::FileLoader(const std::string& filename, const std::string& directory, bool is_gz)
...
template <typename T>
bool FileLoader<T>::CanOpenFile(std::string& /*error*/) {
    return true;
}

template <typename T>
typename FileLoader<T>::ImageRef FileLoader<T>::GetImage() {
    if (!_image) {
        OpenFile(_hdu);
    }
...
template <typename T>
void FileLoader<T>::CloseImageIfUpdated() {
...
template <typename T>
bool FileLoader<T>::ImageUpdated() {
    bool changed(false);
...
template <typename T>
bool FileLoader<T>::HasData(FileInfo::Data dl) const {
    switch (dl) {
...
template <typename T>
bool FileLoader<T>::GetShape(IPos& shape) {
...
template <typename T>
bool FileLoader<T>::GetCoordinateSystem(casacore::CoordinateSystem& coord_sys) {
...
template <typename T>
bool FileLoader<T>::FindCoordinateAxes(IPos& shape, int& spectral_axis, int& z_axis, int& stokes_axis, std::string& message) {
...
template <typename T>
std::vector<int> FileLoader<T>::GetRenderAxes() {
    // Determine which axes will be rendered
    std::vector<int> axes;
...
template <typename T>
bool FileLoader<T>::GetSlice(casacore::Array<float>& data, const casacore::Slicer& slicer) {
    try {
        auto image = GetImage();
...
template <typename T>
bool FileLoader<T>::GetSubImage(const casacore::Slicer& slicer, casacore::SubImage<float>& sub_image) {
...
template <typename T>
bool FileLoader<T>::GetSubImage(const casacore::LattRegionHolder& region, casacore::SubImage<float>& sub_image) {
...
template <typename T>
bool FileLoader<T>::GetBeams(std::vector<CARTA::Beam>& beams, std::string& error) {
...
template <typename T>
const typename FileLoader<T>::IPos FileLoader<T>::GetStatsDataShape(FileInfo::Data ds) {
    throw casacore::AipsError("getStatsDataShape not implemented in this loader");
}

template <typename T>
casacore::ArrayBase* FileLoader<T>::GetStatsData(FileInfo::Data ds) {
    throw casacore::AipsError("getStatsData not implemented in this loader");
}

template <typename T>
void FileLoader<T>::LoadStats2DBasic(FileInfo::Data ds) {
    if (HasData(ds)) {
...
template <typename T>
void FileLoader<T>::LoadStats2DHist() {
...
// TODO: untested
template <typename T>
void FileLoader<T>::LoadStats2DPercent() {
...
template <typename T>
void FileLoader<T>::LoadStats3DBasic(FileInfo::Data ds) {
    if (HasData(ds)) {
        const IPos& stat_dims = GetStatsDataShape(ds);
...
template <typename T>
void FileLoader<T>::LoadStats3DHist() {
...
// TODO: untested
template <typename T>
void FileLoader<T>::LoadStats3DPercent() {
    FileInfo::Data dsr = FileInfo::Data::RANKS;
    FileInfo::Data dsp = FileInfo::Data::STATS_2D_PERCENT;
...
template <typename T>
void FileLoader<T>::LoadImageStats(bool load_percentiles) {
...
template <typename T>
FileInfo::ImageStats& FileLoader<T>::GetImageStats(int current_stokes, int z) {
    return (z >= 0 ? _z_stats[current_stokes][z] : _cube_stats[current_stokes]);
}

template <typename T>
bool FileLoader<T>::GetCursorSpectralData(
...
template <typename T>
bool FileLoader<T>::UseRegionSpectralData(const casacore::IPosition& region_shape, std::mutex& image_mutex) {
    // Must be implemented in subclasses; should call before GetRegionSpectralData
    return false;
}

template <typename T>
bool FileLoader<T>::GetRegionSpectralData(int region_id, int stokes, const casacore::ArrayLattice<casacore::Bool>& mask,
...
template <typename T>
bool FileLoader<T>::GetDownsampledRasterData(
...
template <typename T>
bool FileLoader<T>::HasMip(int mip) const {
    return false;
}

template <typename T>
bool FileLoader<T>::UseTileCache() const {
    return false;
}

template <typename T>
std::string FileLoader<T>::GetFileName() {
    return _filename;
}

template <typename T>
double FileLoader<T>::CalculateBeamArea() {
    auto image = GetImage();
    if (!image) {
        return NAN;
    }
...
template <typename T>
bool FileLoader<T>::GetStokesTypeIndex(const CARTA::PolarizationType& stokes_type, int& stokes_index) {
...
template <typename T>
void FileLoader<T>::SetStokesCrval(float stokes_crval) {
    _stokes_crval = stokes_crval;
}

template <typename T>
void FileLoader<T>::SetStokesCrpix(float stokes_crpix) {
    _stokes_crpix = stokes_crpix;
}

template <typename T>
void FileLoader<T>::SetStokesCdelt(int stokes_cdelt) {
    _stokes_cdelt = stokes_cdelt;
}

template <typename T>
bool FileLoader<T>::SaveFile(const CARTA::FileType type, const std::string& output_filename, std::string& message) {
