...
bool Frame::VectorFieldImage(VectorFieldCallback& partial_vector_field_callback) {
    ...
    if (require_stokes_i && !GetStokesTypeIndex("Ix", stokes_i)) {
        return false;
    }
...
void Frame::GetTiles(int image_width, int image_height, int mip, std::vector<carta::Tile>& tiles) {
    int tile_size_original = TILE_SIZE * mip;
...
void Frame::FillTileData(CARTA::TileData* tile, int32_t x, int32_t y, int32_t layer, int32_t mip, int32_t tile_width, int32_t tile_height,
    ...
    if (tile) {
        ...
        if (compression_type == CARTA::CompressionType::ZFP) {
            ...
            auto nan_encodings = carta::GetNanEncodingsBlock(array, 0, tile_width, tile_height);
            tile->set_nan_encodings(nan_encodings.data(), sizeof(int32_t) * nan_encodings.size());
            // Compress and fill the data
            std::vector<char> compression_buffer;
            size_t compressed_size;
            int precision = lround(compression_quality);
            carta::Compress(array, 0, compression_buffer, compressed_size, tile_width, tile_height, precision);
