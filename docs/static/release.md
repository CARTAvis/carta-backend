...
-# `dev` branch: update `CHANGELOG.md`. Create a new `Unreleased` section.
...
-# `dev` branch: update `CHANGELOG.md`. Change the `Unreleased` heading to `123.0.0`.
-# Merge the `dev` branch into the `release/123.0` branch.
-# `release/123.0` branch: update `VERSION_ID` string in `src/Util/App.h` to `123.0.0-rc.0`.
-# Create a `v123.0.0-rc.0` tag using the release branch.
-# Test the release branch. Make any required fixes in the `dev` branch, and merge them into the release branch. Ideally, bump the version and create a new tag every time changes are merged. If you don't want to bump the version, remember to destroy and recreate the latest tag.
-# `release/123.0` branch: update `VERSION_ID` string in `src/Util/App.h` to `123.0.0`.
-# Create a `v123.0.0` tag using the release branch.
-# Create packages from the release branch.

After final release
-------------------

-# `dev` branch: update `VERSION_ID` string in `src/Util/App.h` to `124.0.0-dev`.
-# `dev` branch: update `CHANGELOG.md`. Create a new `Unreleased` section.
