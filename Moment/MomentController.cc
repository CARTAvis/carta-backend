...
void MomentController::SetMomentGenerator(int file_id, const String& filename, casacore::ImageInterface<float>* image, int spectral_axis,
    int stokes_axis, const CARTA::MomentRequest& moment_request) {
    auto moment_generator =
        std::unique_ptr<MomentGenerator>(new MomentGenerator(filename, image, spectral_axis, stokes_axis, moment_request));
